...
const EXCHANGE_NAME = 'concoxdata';
...
//            var data=adata+'0D0A'
//            logger.info(data)
	arrdata.some(function(adata,index){
	var data=adata+'0D0A'
	logger.info(data)
           ...
           const protocol=str.slice(6,8);
            ...
            switch(protocol){
                case '01':
                    ...
                    if(hid.length==16){
                        channel.sendToQueue(EXCHANGE_NAME, Buffer.from(hid+data));
...
// Make the server a TCP server listening on port 1558.
server.listen(1505, async function () {
	...
	server.on('exit', (code) => {
	logger.info(`Closing with code:`+code);
	});
